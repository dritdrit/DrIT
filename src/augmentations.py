...
def random_conv(x):
	...
	n, c, h, w = x.shape    # x:obs.clone(),128,9,84,84
	for i in range(n):
...
def prepare_pad_batch(obs, next_obs, action, batch_size=32):
	...
	batch_obs = batch_from_obs(torch.from_numpy(obs).cuda(1), batch_size)
	batch_next_obs = batch_from_obs(torch.from_numpy(next_obs).cuda(1), batch_size)
	batch_action = torch.from_numpy(action).cuda(1).unsqueeze(0).repeat(batch_size, 1)
