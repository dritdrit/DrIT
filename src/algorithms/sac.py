...
class SAC(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		shared_cnn = m.SharedCNN(obs_shape, args.num_shared_layers, args.num_filters).cuda(1)    # shared_11,filters_32
		head_cnn = m.HeadCNN(shared_cnn.out_shape, args.num_head_layers, args.num_filters).cuda(1)
		...
		self.actor = m.Actor(actor_encoder, action_shape, args.hidden_dim, args.actor_log_std_min, args.actor_log_std_max).cuda(1)
		self.critic = m.Critic(self.critic_encoder, action_shape, args.hidden_dim).cuda(1)
		self.critic_target = deepcopy(self.critic)

		self.log_alpha = torch.tensor(np.log(args.init_temperature)).cuda(1)
	...
	def _obs_to_input(self, obs):
		if isinstance(obs, utils.LazyFrames):
			...
			_obs = obs
		_obs = torch.FloatTensor(_obs).cuda(1)
