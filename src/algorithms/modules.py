...
def _get_out_shape_cuda(in_shape, layers):
	x = torch.randn(*in_shape).cuda(1).unsqueeze(0)
...
class Actor(nn.Module):
	...
	def forward(self, x, compute_pi=True, compute_log_pi=True, detach=False):
		x = self.encoder(x, detach)    # x.shape:128*100
		mu, log_std = self.mlp(x).chunk(2, dim=-1)    # mu.shape:128*6, log_std.shape:128*6
		...
		mu, pi, log_pi = squash(mu, pi, log_pi)    # log_pi.shape:128*1,选择动作
