...
class ReplayBuffer(object):
    ...
    def sample_soda(self, n=None):
        ...
        return torch.as_tensor(obs).cuda(1).float()
    ...
    def __sample__(self, n=None, for_encoder=False):
        ...
        if for_encoder:
            obs, _ = self._encode_obses(idxs)
            obs = torch.as_tensor(obs).cuda(1).float()
            actions = torch.as_tensor(self.actions[idxs]).cuda(1)
            return obs, actions
        # --------------------------------------------------------
        else:
            obs, next_obs = self._encode_obses(idxs)
            obs = torch.as_tensor(obs).cuda(1).float()
            next_obs = torch.as_tensor(next_obs).cuda(1).float()
            actions = torch.as_tensor(self.actions[idxs]).cuda(1)
            rewards = torch.as_tensor(self.rewards[idxs]).cuda(1)
            not_dones = torch.as_tensor(self.not_dones[idxs]).cuda(1)
    ...
    def sample_encoder(self, n=None, pad=4):
        if self.encoder_multi != 0:
            ...
            return torch.as_tensor([]).cuda(1).float(), torch.as_tensor([]).cuda(1).float()
